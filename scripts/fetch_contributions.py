...
USERNAME = os.environ.get("GH_PROFILE_USER", "sapppu")
...
def fetch_days():
    ...
    for td in cells:
        ...
        text = tooltip_el.get_text(strip=True) if tooltip_el else None
        if not text:
            # fallback: some layouts expose level via data-level only
